...
class Note {
  ...
  // Noteクラスの仕様上オーバーライドが必要だが、このクラスでは使用しないためダミー値を返します
  double getEndPos() { 
    return UNDEFINED;
  }
...
class Music extends Scene {
    ...
    for (int i=0; i<lines.length; i++) {
      ...
      for (int j=0; j<lines[i].length(); j++) {
        ...
        if (status==0 || status==3) {
          ...
          if (current=='#') {
            ...
            //ロングノーツの終端
            Note before;
            if (size==0 || (before=lanes[d][notenum].get(size-1)).getType()==NoteType.NORMAL) throw new ScoreFileSyntaxErrorException(i,"ロングノーツの終点に対応する始点がありません。");
            before.setEndPos(time);
            notenum++;
          } else if (num==0) {
            notenum++;       
          }else{
            throw new ScoreFileSyntaxErrorException(i,"予期されない文字が含まれています："+current);
          }
        } else if (status==1) {
          //コマンド入力中
          if (current==' ') status=2;
      ...
      if (status==-1 || status==-2) {
        ...
        case "DEMOSTART":
          ...
          break;
        default:
          throw new ScoreFileSyntaxErrorException(i,"このヘッダは存在しません。");
        }
        commandName="";
        argument="";
        ...
        case "#MEASURE":
          ...
          break;
        default:
          throw new ScoreFileSyntaxErrorException(i,"この#コマンドは存在しません。");
        }
        commandName="";
        argument="";
  ...
  class ScoreFileSyntaxErrorException extends Exception {
    ScoreFileSyntaxErrorException(int line,String msg) {
      super(msg+"\nline:"+line);
    }
  }
}
